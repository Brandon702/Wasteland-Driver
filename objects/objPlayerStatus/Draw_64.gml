...
if(room != rGameOver)
{
	...
	if !(room == rPauseMenu) && !(room == rMainMenu) && !(room == rWorkshop) && !(room == rGarage && !(room = rGameOver) && !(room= rCredits))
	{
		...
		*/
		...
		{
			//Draw
			draw_sprite_ext(sLowWater,-1,80,450,4,4,0,c_white,1);
			draw_sprite_ext(sLowWaterTransparency,-1,80,450,4,4,0,c_white,-(pWt/50)+1);
		}
		if (pFd <= 50)
		{
			//Draw
			draw_sprite_ext(sLowFood,-1,80,550,4,4,0,c_white,1);
		}
